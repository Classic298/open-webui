<script lang="ts">
	...
	const handlePruneDataConfirm = async (event) => {
		const { days, exempt_archived_chats, exempt_chats_in_folders } = event.detail;
		const res = await pruneData(localStorage.token, days, exempt_archived_chats, exempt_chats_in_folders).catch(
			(error) => {
				toast.error(`${error}`);
				return null;
			}
