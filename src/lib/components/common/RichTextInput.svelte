	...
	onMount(async () => {
		...
		if (!json) {
			if (preserveBreaks) {
				...
				turndownService.addRule('preserveEmptyParagraphWithNbsp', {
					filter: function (node) {
						return node.nodeName === 'P' && node.innerHTML === '&nbsp;';
					},
					replacement: function (content) {
						return '\n \n'; // Or ' \n\n' depending on preference - results in an empty line with a space
					}
				});
			}
			...
			if (!raw) {
				...
				if (preserveBreaks && content) {
					...
					content = content.replace(/<p>(<br\s*\/?>)?<\/p>/g, '<p>&nbsp;</p>');
				}
			}
		} else {
			if (html && !content) {
			...
			if (raw) {
				...
				if (
					...
					if (preserveBreaks) {
						...
						editor.commands.setContent(value);
						if (htmlToSet) { 
                        	htmlToSet = htmlToSet.replace(/<p><\/p>/g, '<p>&nbsp;</p>');
                        	htmlToSet = htmlToSet.replace(/<p>(<br\s*\/?>)?<\/p>/g, '<p>&nbsp;</p>');
                        }
