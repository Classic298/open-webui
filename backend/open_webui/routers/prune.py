...
async def prune_data(form_data: PruneDataForm, user=Depends(get_admin_user)):
    ...
    try:
        ...
        # Stage 2: Prune orphaned data based on a single source of truth: the database.
        ...
        # Prune File records that are not in an active KB or belong to a deleted user.
        for file in Files.get_files():
            if file.id not in active_kb_file_ids or file.user_id not in user_ids:
                try:
                    ...
                    log.warning(f"Vector collection for file {file.id} was already deleted. Cleaning up file record.")
                    if Files.get_file_by_id(file.id):
                        Files.delete_file_by_id(file.id)
                except Exception as e:
                    log.error(f"An error occurred while deleting file {file.id}: {e}")

        # Prune Knowledge Bases belonging to deleted users.
        for kb in all_kbs:
            if kb.user_id not in user_ids:
                try:
                    ...
                    # KB's vector collection was already gone. Proceed with deleting the DB record.
                    log.warning(f"Vector collection for KB {kb.id} was already deleted. Cleaning up KB record.")
                    if Knowledges.get_knowledge_by_id(kb.id):
                        Knowledges.delete_knowledge_by_id(kb.id)
        ...
        # Stage 3: Final Cleanup of transient caches and the main database.
