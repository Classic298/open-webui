...
async def get_file_hash(file: FileModel, persist: bool = False) -> Optional[str]:
    """
...
async def compare_files_for_sync(
    ...
    for incoming_file in form_data.files:
        ...
        if existing_file:
            ...
            existing_hash = await get_file_hash(existing_file, persist=False)
    ...
    for file_id, file_hash in files_needing_hash_persist:
        try:
            await Files.update_file_metadata_by_id(file_id, {'file_hash': file_hash})
        except Exception as e:
