...
async def get_functions(include_valves: bool = False, user=Depends(get_admin_user)):
    return await AsyncFunctions.get_functions(include_valves=include_valves)
...
async def sync_functions(
    ...
    try:
        ...
        return await AsyncFunctions.sync_functions(user.id, form_data.functions)
    except Exception as e:
...
async def create_new_function(
    ...
    function = await AsyncFunctions.get_function_by_id(form_data.id)
    if function is None:
        try:
            ...
            function = await AsyncFunctions.insert_new_function(user.id, function_type, form_data)

            function_cache_dir = CACHE_DIR / "functions" / form_data.id
            function_cache_dir.mkdir(parents=True, exist_ok=True)

            if function_type == "filter" and getattr(function_module, "toggle", None):
                await AsyncFunctions.update_function_metadata_by_id(id, {"toggle": True})
...
async def get_function_by_id(id: str, user=Depends(get_admin_user)):
    function = await AsyncFunctions.get_function_by_id(id)
...
async def toggle_function_by_id(id: str, user=Depends(get_admin_user)):
    function = await AsyncFunctions.get_function_by_id(id)
    if function:
        function = await AsyncFunctions.update_function_by_id(
...
async def update_function_by_id(
    ...
    try:
        ...
        function = await AsyncFunctions.update_function_by_id(id, updated)
...
async def delete_function_by_id(
    ...
    result = await AsyncFunctions.delete_function_by_id(id)
...
async def get_function_valves_by_id(id: str, user=Depends(get_admin_user)):
    ...
    if function:
        try:
            valves = await AsyncFunctions.get_function_valves_by_id(id)
            return valves
        except Exception as e:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if function:
        ...
        if hasattr(function_module, "Valves"):
            ...
            try:
                ...
                await AsyncFunctions.update_function_valves_by_id(id, valves_dict)
                return valves_dict
            except Exception as e:
...
async def get_function_user_valves_by_id(id: str, user=Depends(get_verified_user)):
    ...
    if function:
        try:
            user_valves = await AsyncFunctions.get_user_valves_by_id_and_user_id(id, user.id)
            return user_valves
        except Exception as e:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if function:
        ...
        if hasattr(function_module, "UserValves"):
            ...
            try:
                ...
                await AsyncFunctions.update_user_valves_by_id_and_user_id(
