...
async def get_groups(share: Optional[bool] = None, user=Depends(get_verified_user)):
    ...
    groups = await Groups.get_groups(filter=filter)
    ...
    try:
        ...
        if group:
            return GroupResponse(
                **group.model_dump(),
                member_count=await Groups.get_group_member_count_by_id(group.id),
            )
        else:
            raise HTTPException(
...
async def export_group_by_id(id: str, user=Depends(get_admin_user)):
    ...
    if group:
        return GroupExportResponse(
            **group.model_dump(),
            member_count=await Groups.get_group_member_count_by_id(group.id),
            user_ids=await Groups.get_group_user_ids_by_id(group.id),
        )
    else:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
async def get_users_in_group(id: str, user=Depends(get_admin_user)):
    try:
        users = await Users.get_users_by_group_id(id)
        return users
    except Exception as e:
