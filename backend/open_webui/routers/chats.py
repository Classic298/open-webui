...
async def get_session_user_chat_usage_stats(
    ...
    try:
        ...
        result = await Chats.get_chats_by_user_id(user.id, skip=skip, limit=limit)
...
async def delete_all_user_chats(request: Request, user=Depends(get_verified_user)):
    ...
    # ASYNC: Use Chats for non-blocking delete
...
async def create_new_chat(form_data: ChatForm, user=Depends(get_verified_user)):
    try:
        # ASYNC: Use Chats for non-blocking insert
...
async def import_chats(form_data: ChatsImportForm, user=Depends(get_verified_user)):
    try:
        chats = await Chats.import_chats(user.id, form_data.chats)
        return chats
    except Exception as e:
        log.exception(e)
        raise HTTPException(
...
async def get_chat_by_id(id: str, user=Depends(get_verified_user)):
    # ASYNC: Use Chats for non-blocking get
