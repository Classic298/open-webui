...
class ChatTable:
    ...
    def get_chats_by_user_id_and_search_text(
        self,
        user_id: str,
        search_text: str, # Reverted: No longer Optional, plain string
        include_archived: bool = False,
        skip: int = 0,
        limit: int = 60,
    ) -> list[ChatModel]: # Python 3.9+ list hint

        plain_text_parts: List[str] = []
        tag_ids: List[str] = []
        before_date_str_internal: Optional[str] = None
        after_date_str_internal: Optional[str] = None

        if search_text: # Ensure search_text is not empty before processing
            words = search_text.lower().strip().split(' ')
            for word in words:
                ...
                if word.startswith("tag:"):
                    tag_value = word[4:]
                    if tag_value:
                        tag_ids.append(tag_value.replace(" ", "_").lower())
                elif word.startswith("before:"):
                    date_value = word[7:]
                    if date_value:
                        before_date_str_internal = date_value
                elif word.startswith("after:"):
                    date_value = word[6:]
                    if date_value:
                        after_date_str_internal = date_value
                else:
                    plain_text_parts.append(word)

        plain_search_text = " ".join(plain_text_parts)
        ...
        with get_db() as db:
            ...
            # Text search
            if plain_search_text:
                dialect_name = db.bind.dialect.name
                if dialect_name == "sqlite":
                    text_filter = Chat.title.ilike(f"%{plain_search_text}%") | text(
                        """
                        EXISTS (
                            SELECT 1
                        ...
                        """
                    ).params(search_text=plain_search_text)
            ...
            # Date filtering
            if after_date_str_internal:
                try:
                    dt_after = datetime.strptime(after_date_str_internal, "%Y-%m-%d")
                    ...
                    log.warning(f"Invalid after_date format: {after_date_str_internal}. Expected YYYY-MM-DD.")

            if before_date_str_internal:
                try:
                    dt_before = datetime.strptime(before_date_str_internal, "%Y-%m-%d")
                    ...
                    log.warning(f"Invalid before_date format: {before_date_str_internal}. Expected YYYY-MM-DD.")
