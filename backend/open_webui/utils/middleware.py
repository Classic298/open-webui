...
async def add_file_context(messages: list, chat_id: str, user, include_knowledge: bool = False) -> list:
    ...
    include_knowledge surfaces chat-attached collections and notes (which carry no URL) so
    the model can query them via the builtin knowledge tools; used when File Context is off.
    """
    ...
    for message, stored_message in zip(user_messages, stored_user_messages):
        ...
        if include_knowledge:
            tags += [
                ...
                if item.get('type') in ('collection', 'note') and item.get('id')
            ]
        if not tags:
            continue
