...
async def periodic_session_pool_cleanup():
    ...
    while True:
        if not session_aquire_func():
            log.debug("Session cleanup lock held by another worker. Retrying later.")
            await asyncio.sleep(
                random.uniform(
        ...
        try:
            while True:
                if not session_renew_func():
                    log.warning(
                        "Unable to renew session cleanup lock. Will re-acquire."
                    )
                    break
...
async def periodic_usage_pool_cleanup():
    while True:
        if not aquire_func():
            log.debug("Usage cleanup lock held by another worker. Retrying later.")
            await asyncio.sleep(
                random.uniform(
                    WEBSOCKET_REDIS_LOCK_TIMEOUT / 2, WEBSOCKET_REDIS_LOCK_TIMEOUT
                )
        ...
        try:
            while True:
                if not renew_func():
                    log.warning(
                        "Unable to renew usage cleanup lock. Will re-acquire."
                    )
                    break
